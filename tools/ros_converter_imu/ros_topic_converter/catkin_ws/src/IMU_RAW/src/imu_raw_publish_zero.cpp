...
//	std::cout << k << std::endl;
  ...
  if (it != tow_data.end() && correct_initial_orientation==false)
  {
	...
	imu_raw.angular_velocity.x = 0.0;
	imu_raw.angular_velocity.y = 0.0;
	imu_raw.angular_velocity.z = 0.0;
	imu_raw.linear_acceleration.x = 0.0;
	imu_raw.linear_acceleration.y = 0.0;
	imu_raw.linear_acceleration.z = 0.0;
  	imu_raw.orientation.x = 1.0;
...
int main(int argc, char **argv)
{
  ...
  if (!strcmp(setting.c_str(),"d435i_to_imu_raw")) {
      ...
      ROS_INFO("Read topic : /d435i/imu \nPublish new IMU topic with PADSolution orientation: /imu_raw");
      sub_imu = nh.subscribe("/d435i/imu", 1000, imuWithOrientationCallback);
      pub_imu_raw = nh.advertise<sensor_msgs::Imu>("/imu_raw", 1000);
  }	
