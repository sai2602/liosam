...
double clock_to_gps_secs(ros::Time& stamp)
{
	unsigned int currentLeapSeconds = 18;
    unsigned int seconds = stamp.secs;
    unsigned int secondsSince1970_UTC = seconds;
    secondsSince1980_UTC = secondsSince1970_UTC - 315964800;
    unsigned int secondsGPS = secondsSince1980_UTC + currentLeapSeconds;
    unsigned int secondsPerWeek = 3600 * 24 * 7;
    unsigned int secondsOfWeek = secondsGPS % secondsPerWeek;
    t = ros::Time::Time(secondsOfWeek, stamp.nsecs).toSec();
    return t;
}
/*
...
void pathCallback (const nav_msgs::Path::ConstPtr& path)
{
	...
	float path_tow_secs = clockToGPSsecs(path->header.stamp);
	
	std::cout << "path_tow_secs : " << path_tow_secs << std::endl;
	...
	{	
		...
		path->poses[curr_count-1].pose.position.y << "; " << 
		path->poses[curr_count-1].pose.position.z << ";" <<
		path->poses[curr_count-1].pose.orientation.x << ";" <<
		path->poses[curr_count-1].pose.orientation.y << ";" <<
		path->poses[curr_count-1].pose.orientation.z << ";" <<
		path->poses[curr_count-1].pose.orientation.w << "\n";
...
void sumaPoseCallback(const geometry_msgs::PoseStamped::ConstPtr& suma_pose)
{	
	// convert message header to TOW
	
	//double pose_tow_secs = clockToGPSsecs(suma_pose->header.stamp.sec, suma_pose->header.stamp.nsec);
	
	float pose_tow_secs = float(suma_pose->header.stamp.sec) + float(suma_pose->header.stamp.nsec)*1e-9;
	
	//std::cout << "pose_tow_secs : " << pose_tow_secs << std::endl;
	...
	for(float element : tow_data)
		abs_diffs.emplace_back(std::abs(element-pose_tow_secs));
	...
	{	
		...
		outFile << std::setprecision(15) << tow_data[index] << "; " <<
		suma_pose->pose.position.x << "; " <<
		suma_pose->pose.position.y << "; " << 
		suma_pose->pose.position.z << ";" <<
		suma_pose->pose.orientation.x << ";" <<
		suma_pose->pose.orientation.y << ";" <<
		suma_pose->pose.orientation.z << ";" <<
		suma_pose->pose.orientation.w << "\n";
...
int main(int argc, char** argv)
{
	...
	outFile << "tow; pos_x; pos_y; pos_z; x; y; z; w\n";
	//printPADcsv();
	ros::init(argc, argv, "gps_publisher");
	ros::NodeHandle nh;
	ros::NodeHandle nh_private("~");
	//ROS_INFO("Read PADSolution file and extract data based on timestamp of /lio_sam/mapping/path message");
	...
	sub_sumaPose=nh.subscribe("/pose_suma", 1000, sumaPoseCallback);
