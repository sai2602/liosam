...
std::vector<double3> bStdDev_data;
...
void pathCallback (const nav_msgs::Path::ConstPtr& path)
{
	...
	{	
		...
		outFile<<std::setprecision(10) <<bStdDev_data[index].x << "; " ; 
		
		outFile<<std::setprecision(10) <<bStdDev_data[index].y << "; " ; 
		
		outFile<<std::setprecision(10) <<bStdDev_data[index].z << "; " ;
...
void readPADcsv()
{
      ...
      in.read_header(io::ignore_extra_column, "tow", "lat", "lon", "alt", "accuracy", "ECEFX", "ECEFY", "ECEFZ");
      std::string tow;
      std::string lat;
      std::string lon;
      std::string alt;
      ...
      while(in.read_row(tow, lat, lon, alt, accuracy, bStdDeviation0, bStdDeviation1, bStdDeviation2))
      {
        ...
        try
	    {
      	  ...
      	  bStdDev_data.emplace_back(double3{	std::stold(bStdDeviation0), 
      	  											 	std::stold(bStdDeviation1), 
      	  											 	std::stold(bStdDeviation2)
      	  											 });
        ...
        {
          bStdDev_data.emplace_back(double3{0.0, 0.0, 0.0});        
        }
...
int main(int argc, char** argv)
{
	...
	outFile << "tow; ECEFX; ECEFY; ECEFZ; x; y; z\n";
	//printPADcsv();
